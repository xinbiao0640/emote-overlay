...
- 径向滚轮：鼠标滑动高亮，松开呼出键选中；**按住时滚动鼠标滚轮切换表情分组**
- 自定义表情：支持透明 **PNG**、动态 **GIF**、**WebP**，可上传 / 删除 / 重命名 / 拖动排序
- 多个表情可**同时显示**，每次弹出带随机位置偏移避免重叠
- **连续发送**：鼠标不动时松开呼出键会重复发送上一个表情
- 自定义快捷键（呼出键、关闭键）
- 表情大小 / 显示时长 / 淡入淡出可调
...
4. 回到游戏/桌面，**按住**呼出键（默认 `V`）→ 鼠标滑向某个方向选中对应表情 → **松开**呼出键 → 表情弹出、滚轮关闭。按住期间滚动鼠标滚轮可切换表情分组。
